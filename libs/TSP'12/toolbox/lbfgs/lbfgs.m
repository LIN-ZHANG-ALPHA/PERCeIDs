function [x,fval,exitflag,output] = lbfgs(fun, x0, options, varargin)
...
% TolGrad	Tolerance on gradient L2 norm
...
% Convex    Indicates whether the function is convex
...
% Programming, 1994
%
% $Id: lbfgs.m 27 2011-05-12 01:40:06Z weinman $
  ...
  while (ii<=maxIter)
	...
	[step,x,fval,exitflag,output] = linemin(fun, x, d, lineMinOpt,varargin{:});
	...
	for k=1:(maxIter-ii)
	  ...
	  if (2*abs(fval-fvalold) <= tol*(abs(fval)+abs(fvalold)+eps))
