...
%% matrix case
    ...
    if strcmp( ops.loss, 'ls' )
        for itr = 1:ops.maxitr
           [ H{2}, U{2}, GG, i ] = lsADMMdense( Y , H{1}, H{2}, U{2}, 2, GG, ops);
           his.admm(2,itr) = i;
           [ H{1}, U{1}, GG, i ] = lsADMMdense( Y', H{2}, H{1}, U{1}, 1, GG, ops);
           his.admm(1,itr) = i;
           his.time(itr) = toc;
...
%% dense tensor case
    ...
    for itr = 1:ops.maxitr
        ...
        for d = 1:dim
            dc = [ 1:d-1, d+1:dim ];
            W = khatrirao( H{ dc }, 'r' ); % FORM HERE 
            [ H{d}, U{d}, GG, i ] = lsADMMdense( Ym{d}, W, H{d}, U{d}, d, GG, ops);
            his.admm(d,itr) = i;
        end
        his.time(itr) = toc;
        his.err(itr)  = Loss( Y, H, ops, normY, dim );
